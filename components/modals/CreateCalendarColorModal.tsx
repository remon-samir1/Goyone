...
import { createCalendarColor } from "@/lib/api";

interface CreateCalendarColorModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
}
...
const CreateCalendarColorModal: React.FC<CreateCalendarColorModalProps> = ({
  isOpen,
  onClose,
  onSuccess,
}) => {
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    try {
      await createCalendarColor({
        name,
        color: selectedColor,
      });
      toast.success("Calendar color created successfully");
      onSuccess();
      onClose();
      // Reset form
      setName("");
      ...
      toast.error(error.message || "Failed to create calendar color");
    } finally {
      setLoading(false);
    }
  };
...
export default CreateCalendarColorModal;
